...
export type SourceScore = {
  ...
  isOfficial: boolean;
  /** 1 – 10 numeric score for sorting / tie-breaking */
  numericScore: number;
...
export function scoreSource(result: ProviderResult): SourceScore {
  ...
  let numeric = PROVIDER_BASE_SCORES[result.provider] ?? 5;
  ...
  if (domRel === "high") {
    ...
    numeric = Math.max(1, numeric - 2);
    reasons.push("Low-reliability domain");
  }

  // Map numeric → categorical reliability
  let reliability: SourceReliability;
  if (numeric >= 8) reliability = "high";
  else if (numeric >= 5) reliability = "medium";
  else if (numeric >= 3) reliability = "low";
  ...
  else if (numeric >= 6) confidence = "likely";
  else if (numeric >= 3) confidence = "rumor";
